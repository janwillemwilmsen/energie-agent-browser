-- Allow a `select` step (agent-browser select <ref> <value>) so scenarios can
-- pick an option in a native <select> dropdown. Clicking an `option` node can
-- never work: options of a closed select have no box model (the dropdown list
-- is OS-rendered), so CDP clicks fail with "Could not compute box model".
...
CREATE TABLE scenario_steps_new (
  ...
  kind TEXT NOT NULL CHECK (kind IN (
    'navigate','click','type','fill','select','scroll','screenshot','wait','evaluate',
