-- Allow a `close` step (agent-browser close) so a scenario can tear down the
-- browser session explicitly. SQLite can't ALTER a CHECK constraint, so recreate
-- scenario_steps with the widened kind set. scenario_steps is a leaf table
-- (nothing FK-references it), so recreating it with foreign keys enforced is
-- safe; rows are copied over verbatim.
